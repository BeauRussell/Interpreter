...
type Lexer struct {
	input        string
	position     int  // Position of the cursor checking the input
	readPosition int  // Position to read next in the input
...
func (lexer *Lexer) NextToken() token.Token {
	...
	case '<':
		tok = newToken(token.LANGLE, lexer.ch)
	case '>':
		tok = newToken(token.RANGLE, lexer.ch)
	case ';':
